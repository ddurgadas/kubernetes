...
import (
	...
	"k8s.io/kubernetes/cmd/external-controller-manager/app/options"
...
func NewControllerManagerCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use: "external-controller-manager",
...
func Run(s *options.ExternalCMServer, cloud cloudprovider.Interface) error {
	...
	kubeClient, err := clientset.NewForConfig(restclient.AddUserAgent(kubeconfig, "external-controller-manager"))
	if err != nil {
	...
	recorder := eventBroadcaster.NewRecorder(api.EventSource{Component: "external-controller-manager"})
	...
	rl := resourcelock.EndpointsLock{
		EndpointsMeta: api.ObjectMeta{
			Namespace: "kube-system",
			Name:      "external-controller-manager",
		},
